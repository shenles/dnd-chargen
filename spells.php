     ...
     <table>
      ...
      <?php
        ...
        if (($school == NULL && $level == NULL) && $spellclass == NULL) {
            ...
            $sql = "SELECT name,level,school,casting,spellrange,components,cancast,duration,ritual FROM spells";

        } elseif ($school != NULL) {

            $sql = "SELECT name,level,school,casting,spellrange,components,cancast,duration,ritual FROM spells WHERE INSTR(school, '{$school}') > 0";
 
        } elseif ($level != NULL) {

            $levelstring = strval($level);
            $sql = "SELECT name,level,school,casting,spellrange,components,cancast,duration,ritual FROM spells WHERE INSTR(level, '{$levelstring}') > 0"; 

        } elseif ($spellclass != NULL) {

            $sql = "SELECT name,level,school,casting,spellrange,components,cancast,duration,ritual FROM spells WHERE INSTR(cancast, '{$spellclass}') > 0";
